...
docker run -d --cpuset-cpus="1-2" --restart=always -e MYSQL_ROOT_PASSWORD=nuaactf-ctfd-root -e MYSQL_USER=ctfd -e MYSQL_PASSWORD=ctfd -v $(pwd)/data/mysql:/var/lib/mysql --name=nuaactf-db mariadb:10.2 

#docker rm -f nuaactf-redis
#docker run -d --cpuset-cpus="1-2" --restart=always --name=nuaactf-redis redis:3.2
